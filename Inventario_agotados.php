...
if($cate=="TODOS"){
$query = "SELECT * from inventario where existencia=0 order by categoria,producto asc";    
}else
{
    $query = "SELECT * from inventario where existencia=0 and categoria = :cate order by categoria,producto asc";
}
...
// 	$pdf->SetFont('Arial','',9);			
// 	$pdf->Cell(15,6,utf8_decode($cate),1,0,'C');
...
//while($row = $db->buscar_array($resultado)){
	...
	$pdf->Cell(125,6,utf8_decode (substr($row['producto'], 0,65)),1,0,'L');
