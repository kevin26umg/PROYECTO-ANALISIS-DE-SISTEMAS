...
<html>
  <head>
    <title>Historial Compras al Crédito de Cliente</title>
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            Historial Compras al Crédito de Cliente
            <small>>> </small>
          </h1>
          <ol class="breadcrumb">
            <li><a href="inicio.php"><i class="fa fa-dashboard"></i> Inicio</a></li>
            <li class="active">Historial Compras al Crédito de Cliente</li>
          </ol>
        </section>
...
//                      include "./class_lib/class_conecta_mysql.php";
             $cons= conectar();
            $set_names=$cons->query("SET NAMES 'utf8'");
            ?>
       <div class='form-group'>
                  <label for="encargado" class="col-sm-4 control-label">Cliente:</label> 
                    <div class="col-sm-9">
                    <div>
                      <select name="linea" id="usuario" class="form-control">
                        <?php 
                          $selectLinea=$cons->prepare("SELECT * FROM clientes order by cliente asc");
                          $selectLinea->execute();
                          foreach($selectLinea as $key =>$rowLineas){      
                             echo '<option value="'.$rowLineas['cliente'].'">'.$rowLineas['cliente'].'</option>';
                          }
                        ?>
                      </select>
       ...
       <br>                
        <br>                
           <button class='btn btn-primary pull-left' onclick='mostrar_comision();'><i class='fa fa-search'></i> Consultar</button>
...
window.open("Historial_compras_creditos_cliente_reporte.php?varf="+encodeURIComponent(id)+' &vari=' + encodeURIComponent(idfi)+' &usuario=' + encodeURIComponent(usuario));
}
/*****************************************************************************/
  </script>   
