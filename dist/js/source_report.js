...
function trae_venta(){
  $(document).ready(function(){
    ...
    if(cual=='por_linea'){
      ...
      var data_enviar='fechai='+$("#fi").val()+'&fechaf='+$("#ff").val()+'&usuario='+usuario+'&tipo='+cual;
    }
...
function sumatorias(){
  $(document).ready(function(){
    var detalle=0.00;
	var detalle_traslado=0.00;
    var detalle_cancelado=0.00;
	var detalle_cancelado_traslado=0.00;
    ...
    $('#tabla_ventas_credito > tbody > tr').each(function(){
       ...
       /*tabla de contado*/
        ...
        $('#tabla_ventas_traslado > tbody > tr').each(function(){
                var monto = $(this).find('td').eq(2).html();
                var status = $(this).find('td').eq(3).html();
                monto=monto.replace(",","");
                monto=parseFloat(monto);
                total_traslado+=monto;
             if(status=="CANCELADO"){
               detalle_cancelado_traslado+=monto;
               }
             if(status=="ACTIVA"){
               detalle_traslado+=monto
             }
         $("#total_traslado").html("");
         $("#total_traslado").html("<span class='label label-success'>Total: $ "+total_traslado.toFixed(2)+"</span> | <span class='label label-success'>Cancelaciones: $ "+detalle_cancelado_traslado.toFixed(2)+"</span> | <span class='label label-danger'>Venta Neta: $ "+detalle_traslado.toFixed(2)+"</span>");
        });
       ...
       $("#global").html("<div class='callout callout-success'><h3>Ventas al Cr&eacute;dito: Q"+venta_neta.toFixed(2)+"<br>"+
       "Ventas al Contado: Q"+(detalle_cancelado_contado+detalle_cancelado)+"<br>"+
       "Traslados:   Q"+(detalle_cancelado_traslado+detalle_traslado)+"<br>"+
       "Abonos de Clientes: Q"+total_abonos+"<br>"+
       "Gastos: Q"+total_gastos+"<br>"+
    ...
    if($("#fi").val()!=""||$("#ff").val()!=""){
       $.ajax({
           ...
           /* url: 'busca_los_gastos.php',*/
			url: 'busca_historial_ventas.php',		   
            type: 'POST',
...
/*****************************************************************************/
function mostrar_abonos_cliente(){
 $(document).ready(function (){
    if($("#cliente").val()!=""){
       $.ajax({
            beforeSend: function(){
             $("#data").html("Buscando, espere un momento...");
             },
  			url: 'Abonos_por_cliente_mostrar.php',
            type: 'POST',
      ...
      alert("Ingresa las fechas de busqueda y el Cliente..");
    }
  })
}
/*********************************************************************************/
function mostrar_abonos_proveedor(){
 $(document).ready(function (){
    if($("#proveedor").val()!=""){
       $.ajax({
            beforeSend: function(){
             $("#data").html("Buscando, espere un momento...");
  			...
  			url: 'Abonos_por_proveedor_mostrar.php',
            type: 'POST',
      ...
      alert("Ingresa las fechas de busqueda y el Proveedor..");
    }
  })
}
/*********************************************************************************/
...
/**********************************************************************************/
function abrir_abonos_fecha(){
var vfi=$("#fi").val();
var vff=$("#ff").val();
 window.location.href= 'Abonos_fecha.php?varfi='+encodeURIComponent(vfi) +' &varff=' + encodeURIComponent(vff);
}
/*******************************************************************************************/
function abrir_compras_fecha(){
var vfi=$("#fi").val();
var vff=$("#ff").val();
 window.location.href= 'Compras_fecha.php?varfi='+encodeURIComponent(vfi) +' &varff=' + encodeURIComponent(vff);
}
/*******************************************************************************************/