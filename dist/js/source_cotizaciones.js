      ...
      var cod=$("#codigo").val().trim();
         ...
         $(document).ready(function(){
          ...
          success: function(data){
            ...
            $(".exis").html(data[0].existencia);
            $(".preciol").html(data[0].preciod);
            $("#preciou").attr("disabled", false);
            //$('#preciou').number(true, 2);
            $("#preciou").val(data[0].preciod);
            ...
            $("#preciou").focus();
              if(data[0].imagen!=""){
                ...
                $("#imagen").attr("src",'dist/img/icono7.png');
               }
  ...
  /* registrar_detalle_ventas();*/
   $(document).ready(function(){
         if($("#cantidad").val()>0){
            ...
            var producto=$(".widget-user-desc").html();
            var preciod=$("#preciou").val();
            var cantidad=$("#cantidad").val();
            var total=cantidad*preciod;
			
            $("#tabla_articulos > tbody").append("<tr><td class='center'>"+codigo+"</td><td class='center'>"+producto+"</td><td class='center'>"+cantidad+"</td><td class='center'>"+preciod+"</td><td class='center'>"+total.toFixed(2)+"</td><td class='center'><button class='btn btn-block btn-danger btn-xs delete'><i class='icon-trash bigger-120'></i> Eliminar</button></td></tr>");
            $("#codigo").val("");
            $("#cantidad").val("");
            $("#preciou").val("");
            $("#cantidad").attr("disabled", true);
...
/******************************************************************************************/
function registrar_cotizaciones(){
  ...
  if($("#codigo").val()==""||$("#preciou").val()==""||$("#cantidad").val()==""){
    ...
    $.ajax({
          ...
          type: 'POST',
         data: 'codigo='+codigo+'&producto='+producto+'&precio='+preciod+'&cantidad='+cantidad+'&total='+total+'&factura='+factura+'&departamento='+departamento,
          success: function(x){
              if(x!='0'){
...
/*******************************************************************************************/
function guardar_detalle_ventas(param1,param2,param3,param4,param5,param6,param7,param8,param9,param10){
   var cod=param1;
   var can=param2;
   var preciou=param3;
   var descripcion=param4;
  ...
  /* var factura=$("#factura").val();*/
   $.ajax({
        ...
        url: 'registrar_cotizaciones.php',
        type: 'POST',
		data: 'codigo='+cod+'&cantidad='+can+'&preciou='+preciou+'&descripcion='+descripcion+'&serie='+serie+'&yapuso='+yapuso+'&monto='+monto+'&total='+total+'&supago='+pago+'&cambio='+cambio+'&numero_ticket='+nn,
        success: function(x){
         //alert(x);
