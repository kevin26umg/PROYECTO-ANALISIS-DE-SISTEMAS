    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            <small> Lista de Clientes | Usuario: <?php echo $_SESSION['nombre_de_usuario']; ?> | </small>
...
<br> <br> 
          ...
          <div class="col-sm-4">
          <input type="text" class="form-control cantidades" id='saldolimite' placeholder='' 
...
<br> <br> 
          ...
          <div class="col-sm-4">
          <input type="text" class="form-control" id='plazo' placeholder='' 
          data-inputmask="'placeholder': '0'">
          </div>
...
<br> 
         <div class="form-group">
         <button class='btn btn-azul btn-mini pull-right' id='btn-add' onclick='guardar_abonosx();'><i class='fa fa-save'></i> Guardar abono</button>
         </div>
        <br>
    ...
    var vfactura=0;
    /*********************************************************************************/
    function reporte_historial(id){
    //var usuario=$("#usuario").val();
    window.open("Historial_ventas_credito_cliente_reporte.php?usuario="+encodeURIComponent(id));
//window.open("Historial_ventas_credito_cliente_reporte.php?id="+id);   
 }
/*******************************************************************************************/  
function abonosx(idabonos){
...
if(nit==""||cliente==""){
   ...
   type: 'POST',
   data: 'nit='+$("#nit").val()+'&cliente='+$("#nombre").val()+'&contacto='+$("#contacto").val()+'&direccion='+$("#direccion").val()+'&telefono='+$("#telefono").val()+'&correo='+$("#correo").val()+'&descuento='+$("#descuento").val()+'&saldolimite='+$("#saldolimite").val()+'&plazo='+$("#plazo").val()+'&saldo='+$("#saldo").val()+'&id='+$("#id").val(),
   success: function(x){
...
/*******************************************************************************************/     
function busca_creditos(id){
	var artbuscar=id;
    $.ajax({
        beforeSend: function(){
          $("#data").html("<img src='dist/img/default.gif'></img>");
          },
        url: 'busca_lista_clientes_credito.php',
        type: 'POST',
...
$('#modal_abonos').modal('toggle');  
   ...
   type: 'POST',
   data: 'id='+$("#idcliente").val()+'&cliente='+$("#nombrecliente").val()+'&saldoabono='+$("#saldoabono").val()+'&abono='+$("#abono").val()+'&saldofinalabonos='+$("#saldofinalabonos").val()+'&factura='+vfactura,
   success: function(x){
   alert("El dato se registro satisfactoriamente...!");
   busca();
   },
